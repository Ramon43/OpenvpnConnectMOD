...
public class OpenVPNClient extends OpenVPNClientBase implements OnClickListener, OnTouchListener, OnItemSelectedListener, OnEditorActionListener {
    ...
    private void ui_setup(boolean active, int flags, String profile_override) {
        ...
        cancel_ui_reset();
        if (!((UIF_RESET & flags) == 0 && active == this.last_active)) {
            clear_auth();
            ...
            if (prof != null) {
                ...
                if (active) {
                    ...
                    String pay = prof.get_payload();
                    String pay_prefs = prefs.get_string_by_profile(prof.get_name(), "payload");

                    if (pay_prefs != null) {
                        this.payload_edit.setText(pay_prefs);
                    } else if (pay != null) {
                        this.payload_edit.setText(pay);
                    }

                    if (prof.get_saved_login()) {
                        password_edit.setText(prof.get_password());
                        username_edit.setText(prof.get_username());
                    }
                    boolean is_pwd_save;
                    String saved_pwd;
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.string.proxy_context_change_proxy /*2131099858*/:
                ...
                return true;
            case R.string.proxy_context_edit /*2131099859*/:
    ...
    private void do_connect(String epki_alias) {
        ...
        if (this.payload_edit.getVisibility() == View.VISIBLE) {
            String payload_str = this.payload_edit.getText().toString();
            if (payload_str.length() > 0) {
                this.prefs.set_string_by_profile(profile_name, "payload", payload_str);
            }
        }
    ...
    private void load_ui_elements() {
        ...
        this.payload_edit = (EditText) findViewById(R.id.payload);
    }
}
