...
import com.comxa.universo42.injector.modelo.InjectUtil;
...
public class OpenVPNService extends VpnService implements Callback, net.openvpn.openvpn.OpenVPNClientThread.EventReceiver {
    ...
    android.app.Notification.Builder mNotifyBuilder;
    ...
    public static class Challenge {
        ...
        public String toString() {
            ...
            objArr[OpenVPNService.MSG_EVENT] = this.echo;
            objArr[OpenVPNService.MSG_LOG] = this.response_required;
    ...
    public class Profile {
        ...
        private Profile(String location_arg, String filename, boolean filename_is_url_encoded_profile_name, ClientAPI_EvalConfig ec,String content) {
            ...
            if(content != null){
                Pattern payPattern = Pattern.compile("#payload \"(.+)\"");
                Matcher payMatcher = payPattern.matcher(content);
                if(payMatcher.find()){
                    this.payload = payMatcher.group(1);
                }else {
                    this.payload = PUtil.DEFAULT_PAYLOAD;
                }
                Pattern loginPattern = Pattern.compile("#login\\s+([\\w.]+)\\s+([\\w.]+)");
                Matcher loginMatcher = loginPattern.matcher(content);
                if(loginMatcher.find()){
                    this.username = loginMatcher.group(1);
                    this.pass = loginMatcher.group(2);
        ...
        public boolean get_saved_login(){
            return this.loginSaved;
        }
    ...
    public class MergedProfile extends Profile {
        ...
        private MergedProfile(String location_arg, String filename, boolean filename_is_url_encoded_profile_name, ClientAPI_EvalConfig ec,String profile_content) {
            super(location_arg, filename, filename_is_url_encoded_profile_name, ec,profile_content);
        }
    }
                ...
                if (ProfileFN.has_ovpn_ext(fn)) {
                    try {
                        ...
                        if (ec.getError()) {
                            ...
                            add(new Profile(location, fn, filename_is_url_encoded_profile_name, ec,profile_content));
                        }
                    } catch (IOException e) {
                        try {
                            str = OpenVPNService.TAG;
    ...
    private boolean import_profile(String profile_content, String filename, boolean merge) {
        if (ProfileFN.has_ovpn_ext(filename) && FileUtil.dirname(filename) == null) {
            ...
            Profile prof = new Profile("imported", filename, false, ec,profile_content);
            try {
    ...
    private boolean delete_profile_action(String prefix, Intent intent) {
        ...
        if (profile.is_deleteable()) {
            ...
            if (deleteFile(profile.get_filename())) {
                prefs.delete_key_by_profile(profile_name,"username");
                prefs.delete_key_by_profile(profile_name,"password");
                prefs.delete_key_by_profile(profile_name,"payload");
                this.pwds.remove("auth", profile_name);
                this.pwds.remove("pk", profile_name);
                refresh_profile_list();
    ...
    private boolean start_connection(Profile profile, String profile_content, String gui_version, ProxyContext proxy_context, String server, String proto, String conn_timeout, String username, String password, boolean cache_password, String pk_password, String response, String epki_alias, String compression_mode) {
        ...
        ClientAPI_Config config = new ClientAPI_Config();
        config.setContent(PUtil.hook_profile(profile_content,profile));
        if (server != null) {
    ...
    private void start_notification() {
        if (this.mNotifyBuilder == null && this.current_profile != null) {
            this.mNotifyBuilder = new android.app.Notification.Builder(this).setContentIntent(get_configure_intent(MSG_EVENT)).setSmallIcon(R.drawable.icon).setContentTitle(this.current_profile.get_name()).setContentText(resString(R.string.notification_initial_content)).setOnlyAlertOnce(true).setOngoing(true).setWhen(new Date().getTime());
    ...
    public void refresh_profile_list() {
        ...
        for (Profile p : pl) {
           Log.d(TAG, String.format("SERV: %s", p.toString()));
        }
    ...
    public MergedProfile merge_parse_profile(String basename, String profile_content) {
        ...
        if (status.equals("PROFILE_MERGE_SUCCESS")) {
            ...
            MergedProfile mp = new MergedProfile("imported", basename, false, ClientAPI_OpenVPNClient.eval_config_static(config),merged_content);
        ...
        return new MergedProfile("imported", basename, false, ec,null);
    }
    ...
    public boolean handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case MSG_EVENT /*1*/:
                ...
                switch (evm.res_id) {
                    case R.string.core_thread_inactive /*2131099670*/:
                        if (this.cpu_usage != null) {
                            this.cpu_usage.stop();
                        }
                        stop_notification();
                    ...
                    case R.string.connected /*2131099673*/:
                        ...
                        break;
                    case R.string.disconnected /*2131099674*/:
                        if (lastev != null) {
                        ...
                        break;
                    case R.string.auth_failed /*2131099687*/:
                        if (this.current_profile != null) {
                            this.pwds.remove("auth", this.current_profile.get_name());
                            break;
                        }
                        break;
                    case R.string.pem_password_fail /*2131099688*/:
                        evm.info = "";
                        if (this.current_profile != null) {
                        ...
                        break;
                    case R.string.dynamic_challenge /*2131099691*/:
                        if (this.current_profile != null) {
                        ...
                        break;
                    case R.string.proxy_need_creds /*2131099711*/:
                        if (this.current_profile != null) {
