...
public abstract class OpenVPNClientBase extends AppCompatActivity implements EventReceiver {
    ...
    protected void submitImportProfileViaPathIntent(String profile_path) {
        startService(new Intent(this, OpenVPNService.class).setAction(OpenVPNService.ACTION_IMPORT_PROFILE_VIA_PATH).putExtra(OpenVPNService.INTENT_PREFIX + ".PATH", profile_path));
    }

    protected void submitDeleteProfileIntent(String profile_name) {
       startService(new Intent(this, OpenVPNService.class).setAction(OpenVPNService.ACTION_DELETE_PROFILE).putExtra(OpenVPNService.INTENT_PREFIX + ".PROFILE", profile_name));
    }
    ...
    protected static String render_duration(int duration) {
        ...
        return String.format("%d:%02d:%02d", duration / 3600, minutes, seconds);
    }
